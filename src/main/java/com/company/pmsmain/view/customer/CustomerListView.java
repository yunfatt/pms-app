...
public class CustomerListView extends StandardListView<Customer> {
    ...
    public void onBeforeShow(BeforeShowEvent event) {
        customersDl.setParameter("searchText", "");
        customersDl.load();
    }
        ...
        try {
            ...
            switch (exportType) {
                case PDF -> {
                    ...
                    downloader.download(out.toByteArray(), "CustomerList.pdf", DownloadFormat.PDF);
                }
                case EXCEL -> {
                    StiExportManager.exportExcel(report, out);
                    downloader.download(out.toByteArray(), "CustomerList.xlsx", DownloadFormat.XLSX);
                }
            }
            ...
            notifications.create("Failed to generate report: " + ex.getMessage())
                    ...
                    .show();
            throw new RuntimeException("Failed to generate Customer List report", ex);
        }
    }
    ...
    private StiReport buildReport() throws Exception {
        ...
        File reportFile = loadReportTemplate("CustomerList.mrt");
        ...
        Collection<Customer> items = customersDataGrid.getItems().getItems();
        String propertyNoFrom = "";
        String propertyNoTo   = "ZZZZZZZZZZ";
        if (!items.isEmpty()) {
            List<Customer> sorted = items.stream()
                    .filter(c -> c.getPropertyno() != null)
    ...
    private void executeSearch() {
        ...
        customersDl.setParameter("searchText", text == null ? "" : text.trim());
        customersDl.load();
    }
    ...
    public void onAdvancedSearchToggle(ClickEvent<JmixButton> event) {
        ...
        if (!advancedMode) {
            ...
            customersDl.removeParameter("searchText");
        }
    }
}
