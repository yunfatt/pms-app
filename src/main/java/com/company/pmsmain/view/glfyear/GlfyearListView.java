...
public class GlfyearListView extends StandardListView<Glfyear> {
    ...
    @ViewComponent
    private DataGrid<Glfyear> glfyearDataGrid;
        ...
        try {
            ...
            switch (exportType) {
                case PDF -> {
                    ...
                    downloader.download(out.toByteArray(), "GlfyearList.pdf", DownloadFormat.PDF);
                }
                case EXCEL -> {
                    StiExportManager.exportExcel(report, out);
                    downloader.download(out.toByteArray(), "GlfyearList.xlsx", DownloadFormat.XLSX);
                }
            }
            ...
            notifications.create("Failed to generate report: " + ex.getMessage())
                    ...
                    .show();
            throw new RuntimeException("Failed to generate Fiscal Year List report", ex);
        }
    }
    ...
    private StiReport buildReport() throws Exception {
        ...
        File reportFile = loadReportTemplate("GlfyearList.mrt");
        ...
        Collection<Glfyear> items = glfyearDataGrid.getItems().getItems();
        String fyFrom = "";
        String fyTo   = "ZZZZZZZZZZ";
        if (!items.isEmpty()) {
            List<Glfyear> sorted = items.stream()
                    .filter(g -> g.getId() != null && g.getId().getFiscalyear() != null)
                    .sorted((a, b) -> a.getId().getFiscalyear().compareTo(b.getId().getFiscalyear()))
                    .toList();
            if (!sorted.isEmpty()) {
                fyFrom = sorted.get(0).getId().getFiscalyear();
                fyTo   = sorted.get(sorted.size() - 1).getId().getFiscalyear();
            }
        }
        ...
        report.setVariable("FiscalYearFrom", fyFrom);
        report.setVariable("FiscalYearTo",   fyTo);
