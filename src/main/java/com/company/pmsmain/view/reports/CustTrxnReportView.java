...
@Route(value = "cust-trxn-report", layout = MainView.class)
@ViewController(id = "CustTrxnReport.view")
@ViewDescriptor(path = "cust-trxn-report-view.xml")
public class CustTrxnReportView extends StandardView {
    ...
    private String propertyNo2 = "ZZZ";
    private String dateFrom    = LocalDate.now().withDayOfMonth(1)
            .format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
    private String runDate     = LocalDate.now()
            .format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
    ...
    private void showParameterDialog() {
        ...
        H4 title = new H4("Customer Transactions — Parameters");
        ...
        propertyNo2Field.setValue(propertyNo2.equals("ZZZ") ? "" : propertyNo2);
        ...
        // ── Date From ────────────────────────────────────────────────────────
        DatePicker dateFromPicker = new DatePicker("Date From");
        dateFromPicker.setI18n(i18n);
        dateFromPicker.setWidthFull();
        dateFromPicker.setValue(LocalDate.now().withDayOfMonth(1));
        dateFromPicker.setRequired(true);

        // ── Run Date (Date To) ───────────────────────────────────────────────
        DatePicker runDatePicker = new DatePicker("Date To (Run Date)");
        ...
        nextBtn.addClickListener(e -> {
            if (dateFromPicker.getValue() == null) {
                notifications.create("Date From is required.")
                        .withType(Notifications.Type.WARNING)
                        .show();
                return;
            }
            if (runDatePicker.getValue() == null) {
                notifications.create("Date To is required.")
                        .withType(Notifications.Type.WARNING)
                        .show();
                return;
            }
            if (dateFromPicker.getValue().isAfter(runDatePicker.getValue())) {
                notifications.create("Date From cannot be after Date To.")
                        .withType(Notifications.Type.WARNING)
                        .show();
                return;
            }
...
// ── Property lookup ───────────────────────────────────────────────────────
        ...
        try {
            ...
            switch (exportType) {
                case PDF -> {
                    ...
                    downloader.download(out.toByteArray(), "CustTrxn.pdf", DownloadFormat.PDF);
                }
                case EXCEL -> {
                    StiExportManager.exportExcel(report, out);
                    downloader.download(out.toByteArray(), "CustTrxn.xlsx", DownloadFormat.XLSX);
                }
            }
            ...
            notifications.create("Failed to generate report: " + ex.getMessage())
                    ...
                    .show();
            throw new RuntimeException("Failed to generate Customer Transactions report", ex);
        }
    }
    ...
    private StiReport buildReport() throws Exception {
        ...
        File reportFile = loadReportTemplate("pm_custtrxn.mrt");
        ...
        report.setVariable("PropertyNo2", propertyNo2.isBlank() ? "ZZZ" : propertyNo2);
        report.setVariable("DateFrom",    dateFrom);
        report.setVariable("RunDate",     runDate);
        report.setVariable("UseDueDate",  useDueDate);

        System.out.println(">>> CustTrxn Variables:");
        System.out.println("    PropertyNo1 = [" + propertyNo1 + "]");
        System.out.println("    PropertyNo2 = [" + propertyNo2 + "]");
        System.out.println("    DateFrom    = [" + dateFrom + "]");
        System.out.println("    RunDate     = [" + runDate + "]");
        System.out.println("    UseDueDate  = [" + useDueDate + "]");

        // 6. Render
        report.render();
        System.out.println(">>> Pages rendered: " + report.getRenderedPages().size());
